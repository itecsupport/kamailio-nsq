...
static int
multi_timer_cb(CURLM *multi, long timeout_ms, void *arg)
{
...
static void
check_multi_info(struct HttpClient *httpc)
{
    char *eff_url;
    CURLMsg *msg;
    int msgs_left;
...
static size_t
write_cb(char *ptr, size_t size, size_t nmemb, void *arg)
{
...
static int
sock_cb(CURL *e, curl_socket_t s, int what, void *arg, void *sock_arg)
{
    struct HttpClient *httpc = (struct HttpClient *)arg;
...
void
free_http_client(struct HttpClient *httpc)
{
    if (httpc) {
...
void
free_http_request(struct HttpRequest *req)
{
    if (req) {
...
struct HttpResponse*
new_http_response(int status_code, void *data)
{
    struct HttpResponse *resp;
...
void
free_http_response(struct HttpResponse *resp)
{
    if (resp) {
        free(resp);
    }
...
int
http_client_get(struct HttpClient *httpc, struct HttpRequest *req)
{
    CURLMcode rc;
