...
nsq_lookupd_request_cb(struct HttpRequest *req, struct HttpResponse *resp, void *arg)
{
    ...
    jsobj = json_tokener_parse_ex(jstok, resp->data->data, resp->data->length);
    if (!jsobj) {
    ...
    if (!json_object_object_get_ex(jsobj, "data", &data)) {
        _DEBUG("%s: error getting 'data' key\n", __FUNCTION__);
        json_object_put(jsobj);
        json_tokener_free(jstok);
        return;
    }
    if (!json_object_object_get_ex(data, "producers", &producers)) {
...
new_nsqlookupd_endpoint(const char *address, int port)
{
...
void
free_nsqlookupd_endpoint(struct NSQLookupdEndpoint *nsqlookupd_endpoint)
{
